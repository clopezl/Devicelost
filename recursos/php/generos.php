...
function crea_genero($nombre){
	if(!valida_nombre_genero($nombre)){
		...
		if(mysql_query($sql,connect())){
			...
			serror('Error de mysql al crear el gŽnero');
			return false;
		}
	}
	disconnect();
...
function edita_genero($id,$nuevo_nombre){
	if(!check_genero("id",$id) or !valida_nombre_genero($nuevo_nombre)){
		serror('No se ha encontrado el gŽnero para editar');
		return false;
	}else{
		...
		if(mysql_query($sql,connect())){
			...
			serror('Error de mysql al editar el gŽnero');
			return false;
		}
	}
	disconnect();
...
function elimina_genero($clave,$valor){
	...
	if(!check_genero($clave,$valor)){
		serror('No se ha encontrado el gŽnero para eliminar');
		return false;
	}else{
